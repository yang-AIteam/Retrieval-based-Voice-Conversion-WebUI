...
## Step 0 实测结果与分析（2087 对，seed=0，抽取 30 对）

### 数值汇总

| 方法 | RPA% | RCA% | VR% | cents 中位误差 | lag 中位 |
|---|---|---|---|---|---|
| **rmvpe** | **88.6** | 88.9 | 98.7 | **10.3** | 0 |
| crepe | 74.1 | 74.5 | 81.7 | 16.1 | 0 |
| harvest | 60.8 | 61.6 | 90.4 | 27.8 | 0 |

### 关键发现

**1. RMVPE 完胜——方法选择已定**

RMVPE 未经任何微调就达到 88.6% RPA，远超 Harvest/Crepe。Harvest 在接触式 sensor 上崩得最厉害（谐波结构与其假设差距过大）。**Harvest/Crepe 可以放弃，只用 RMVPE。**

**2. 零八度错误**

所有方法 RPA ≈ RCA（88.6 vs 88.9）。八度跳变会导致 RCA 明显高于 RPA，这里几乎没有差距。这意味着**调整阈值或八度后处理无效**——剩余误差是真实的细音高偏差，不是八度混淆。

**3. lag = 0，时间对齐无问题**

全部 30 对的 lag 中位数均为 0，sensor 与 mic 在帧级别上已经对齐。**后续微调无需做时间校正**，标签可直接逐帧使用。

**4. VR=98.7%，有声帧检测近乎完美**

~11% 的 RPA 缺口不是漏检 voicing，而是**少数帧的音高偏差 >50 cents**。中位误差仅 10.3 cents（人耳基本无感知）。误差分布是"大部分极准 + 一小撮尾巴偏大"的形态。

**5. 薄弱点集中在特定说话人/录音条件**

| ID 前缀 | 各文件 RPA |
|---|---|
| `pUaQKQSh7E` | 84.8 / 78.8 / 83.0 / **69.8** |
| `chc_pimJ0Z` | 86.6 / 83.7 / **76.2** |
| `6ft7Ezz5sQ` | 93.4 / 97.1 / 93.2（稳定高） |

问题不是全局的，而是部分说话人 sensor SNR 差或谐波较弱所致。

### 结论与下一步行动

**88.6% 处于"可用但不完美"的边界，建议先做听感测试再决定是否微调：**

- ground truth 是 RMVPE-on-mic，GT 本身也有误差，真实精度高于 88.6%；
- 零八度错误意味着不会出现灾难性的音高跳变 artifact；
- 中位误差 10.3 cents 听感上无差别。

**建议的行动顺序：**

1. **听感 A/B 测试**（最关键）——重点拿 `pUaQKQSh7E` 等低 RPA 例跑完整 RVC，听是否有音高漂移或抖动；
2. 将 `N_PAIRS` 调到 100、换几个 seed 再跑，确认 88.6% 的统计稳定性；
3. 听感无问题 → 直接上线，无需微调；听感有问题 → 进入 Step 1 蒸馏。

**若决定微调，条件非常有利：** lag=0 标签对应自明，2087 对数据充足，RMVPE 结构已够强，蒸馏主要补弱说话人的尾巴误差，mid-90s RPA 可期。
